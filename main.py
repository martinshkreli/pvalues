...
def get_user_input():
 try:mean1=float(input("Enter the mean ADAS-Cog change from baseline for simufilam: "))
 except ValueError:
 ...
 while mean1<0:
  print("Invalid mean. Please enter a positive value.")
  mean1=float(input("Enter the mean ADAS-Cog change from baseline for simufilam: "))
 try:stddev1=float(input("Enter the standard deviation of the ADAS-Cog change from baseline for simufilam: "))
 except ValueError:
 ...
 while stddev1<0:
  print("Invalid standard deviation. Please enter a positive value.")
  stddev1=float(input("Enter the standard deviation of the ADAS-Cog change from baseline for simufilam: "))
 try:dropout_rate=float(input("Enter the dropout rate (as a decimal): "))
 except ValueError:
 ...
 try:mean2=float(input("Enter the mean ADAS-Cog change from baseline for placebo: "))
 except ValueError:
  print("Invalid mean. Please enter a positive value.")
 ...
 return mean1,stddev1,n1,mean2,stddev2,n2,42,42,use_random_seed,max_delta
...
def main():
 ...
 for _ in range(simulation_size):
  sample1,sample2=generate_samples(mean1,stddev1,n1,mean2,stddev2,n2,seed1,seed2,use_random_seed,do_not_print_details,max_delta)
  _,p_val=perform_t_test(sample1,sample2)
  p_values.append(p_val)
 avg_p_value=np.mean(p_values)
 max_p_value=np.max(p_values)
 min_p_value=np.min(p_values)
 stddev_p_value=np.std(p_values)
 estimated_power=sum(p<0.05 for p in p_values)/simulation_size
 print(f"\nSimulation Results ({simulation_size} runs):")
 ...
 print(f"Estimated Power (p<0.05):{estimated_power:.2%}")
